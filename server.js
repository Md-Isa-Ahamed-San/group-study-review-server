  ...
  try {
    ...
    res.json({
      token: {
        ...
        refreshToken: refreshToken, // Send a fresh refresh token
      },
    });
  } catch (error) {
...
app.get("/api/classes/user/:email", verifyAccessToken, async (req, res) => {
  ...
  console.log("inside Fetch classes associated with a user's email");
  try {
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const isMember = foundClass.members.some(
        (id) => id.toString() === userId
      );
      const isExpert = foundClass.experts.some(
        (id) => id.toString() === userId
      );
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!taskIds || taskIds.length === 0) {
        return res
          .status(404)
          .json({ message: "No tasks found for this class" });
      }
